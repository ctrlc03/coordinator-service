...
/// Util function to download a file from a URL
...
/// Register a new circuit - we trust the users to provide valid URLs
...
/// Get the circuit that are registered
...
/// Get the number of proofs for a certain circuit
...
/// Update the proof status on the db
...
/// Check if a circuit exists by using its name
...
/// Get the proof status and details
...
func (p Prover) GenerateProof(c *gin.Context) {
	...
	// Return before go routine finishes so that the client knows the proof id 
	c.JSON(http.StatusOK, gin.H{
		"status":  http.StatusOK,
		"message": "Proof generation started for proof: " + strconv.Itoa(proofID),
	})
}
...
// Post request to get multiple circuits
...
/// Get the result for a certain circuit
...
/// Fetch all proofs from the db (available proofs)
...
/// Get all proofs generated by the service (can be a lot of data)
